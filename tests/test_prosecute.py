...
def test_extended_prosecutor_end_to_end_against_the_full_fixture_set(labelled_fixtures):
    ...
    assert report["unproven"] == 0, "every filed claim must cite sufficient evidence"
    assert report["rejected"] == 0, "the prosecutor must never emit a schema-invalid or over-quota claim"

    # Added detectors must improve on the starter's 1/17 recall without
    # sacrificing claim quality.
    ...
    # At least one former stub must now produce fully verified claims.
    extended = [
        stats
        for cls, stats in report["per_class"].items()
        if cls != "enforcement_failure" and stats["claimed"] > 0
    ]
    assert extended
    assert all(stats["claimed"] == stats["verified"] for stats in extended)
